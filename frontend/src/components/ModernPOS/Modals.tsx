...
export const DailyStockStatusModal: React.FC<DailyStockStatusModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchStockStatus = async () => {
      ...
      try {
        ...
        const res = await fetch(`${Api}/products/daily-status?date=${todayStr}`);
        if (res.ok) {
          const data = await res.json();
          if (Array.isArray(data)) {
            setStockList(data.map((item: any) => ({
              code: item.itemCode || item.barcode || 'N/A',
              name: item.name || 'Unnamed Product',
              current: item.closingStock ?? item.stock ?? 0,
              status: item.status || (item.closingStock <= 0 ? 'Out of Stock' : item.closingStock < 10 ? 'Low Stock' : 'In Stock')
            })));
          }
        }
      } catch (err) {
        console.error("Failed to fetch daily stock status", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg apple-glass-card rounded-2xl p-6 shadow-2xl z-10 space-y-4 border border-white/80 animate-scale-up">
        <div className="flex items-center justify-between pb-3 border-b border-slate-200/80">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-blue-50 text-blue-600">
              <Package size={20} />
